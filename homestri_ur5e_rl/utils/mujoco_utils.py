...
def get_obs_by_name(model, data, joint_names):
    """Returns all joint positions and velocities associated with a robot."""
    if data.qpos is not None and joint_names:
        return (
            np.squeeze(np.array([get_joint_qpos_by_name(model, data, name) for name in joint_names])),
            np.squeeze(np.array([get_joint_qvel_by_name(model, data, name) for name in joint_names])),
        )
    return np.zeros(0), np.zeros(0)

def get_obs_by_id(model, data, joint_ids):
    """Returns all joint positions and velocities associated with a robot."""
    if data.qpos is not None and joint_ids:
        return (
            np.squeeze(np.array([get_joint_qpos_by_id(model, data, id) for id in joint_ids])),
            np.squeeze(np.array([get_joint_qvel_by_id(model, data, id) for id in joint_ids])),
        )
    return np.zeros(0), np.zeros(0)
...
def get_site_jacp_by_id(model, data, site_id):
...
def get_site_jacp_by_name(model, data, name):
    """Return the Jacobian' translational component of the end-effector of
    the corresponding site id.
    """
...
def get_site_jacr_by_id(model, data, site_id):
...
def get_site_jacr_by_name(model, data, name):
    """Return the Jacobian' rotational component of the end-effector of
    the corresponding site id.
    """
    site_id = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_SITE, name)
...
def get_joint_qpos_by_name(model, data, name):
...
def get_joint_qpos_by_id(model, data,joint_id):
    """Return the joints position and orientation (qpos) of the model."""
    joint_type = model.jnt_type[joint_id]
    joint_addr = model.jnt_qposadr[joint_id]
...
def get_joint_qvel_by_name(model, data, name):
...
def get_joint_qvel_by_id(model, data, joint_id):
    """Return the joints linear and angular velocities (qvel) of the model."""
    joint_type = model.jnt_type[joint_id]
    joint_addr = model.jnt_dofadr[joint_id]
...
def get_site_xpos_by_name(model, data, name):
    site_id = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_SITE, name)
    return data.site_xpos[site_id]

def get_site_xpos_by_id(model, data, site_id):
    return data.site_xpos[site_id]

def get_site_xvelp_by_name(model, data, name):
    jacp = get_site_jacp_by_name(model, data, name)
    xvelp = jacp @ data.qvel
    return xvelp

def get_site_xvelp_by_id(model, data, site_id):
    jacp = get_site_jacp_by_id(model, data, site_id)
    xvelp = jacp @ data.qvel
    return xvelp


def get_site_xvelr_by_name(model, data, name):
    jacp = get_site_jacr_by_name(model, data, name)
    xvelp = jacp @ data.qvel
    return xvelp

def get_site_xvelr_by_id(model, data, site_id):
    jacp = get_site_jacr_by_id(model, data, site_id)
    xvelp = jacp @ data.qvel
    return xvelp
...
def get_site_xmat_by_name(model: MjModel, data: MjData, name: str):
    site_id = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_SITE, name)
    return data.site_xmat[site_id].reshape(3, 3)

def get_site_xmat_by_id(model: MjModel, data: MjData, site_id: int):
    return data.site_xmat[site_id].reshape(3, 3)
